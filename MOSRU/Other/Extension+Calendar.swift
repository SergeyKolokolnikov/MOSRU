...
extension Calendar {
    ...
    func yesterday(_ date: Date) -> Date {
        return self.date(byAdding: .day, value: -1, to: noon(date))!
    }

    func tomorrow(_ date: Date) -> Date {
        return self.date(byAdding: .day, value: 1, to: noon(date))!
    }

    func dayAfter(_ date: Date, days: Int) -> Date {
        return self.date(byAdding: .day, value: days, to: noon(date))!
    }
